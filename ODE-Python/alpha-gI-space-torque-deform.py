...
def deform_spring_byTorque(alphaCoeffs, cICoeffs, torqueTarg):
    ...
    #  print("betaorg",betaorg)
    F = torqueTarg/(rorg*np.sin(betaorg))
    Fx = -np.sin(alpha(fullArcLength, alphaCoeffs))*F
    Fy = -np.cos(alpha(fullArcLength, alphaCoeffs))*F
    print("original guess:", Fx, Fy, lin.norm(F))

    gamma0 = np.array([0, dgds0])
    rErr, gammaErr, dBeta = forward_integration_result(deform_ODE, alphaCoeffs, cICoeffs, gamma0, Fx, Fy, smesh, True, itr)
    ...
    while err>1.1e-6:
        ...
        err = lin.norm(errVec)
        # print(err)
        if err>lin.norm(errVecPrev):
            print("diverging")

        if err<1.1e-6 or itr>5000:
