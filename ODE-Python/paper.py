...
for i in range (-Frange,Frange+1):
    ...
    s_span  = [0, smax]
    gamma = np.zeros(2)
    print("thinking about what happens when F =",i)
...
plt.figure(2)
i = 0
final_angles = np.empty(len(range(-Frange,Frange+1)))
for key in Fangles:
    final_angles[i]=Fangles[key]
plt.plot(np.linspace(-Frange,Frange,len(range(-Frange,Frange+1))), final_angles)
